...
# syntax) or not caught at all (e.g., if the server or solution hangs).
#
# Run this as:
# python interactive_runner.py <cmd_line_server> -- <cmd_line_solution>
#
# For example:
...
# - server's stdin is connected to the solution's stdout;
# - server's stdout is connected to the solution's stdin;
# - stderrs of both server and solution are piped to standard error stream, with
#   lines prepended by "server: " or "sol: " respectively (note, no
...
t_sol = SubprocessThread(sol_args, stderr_prefix="client debug: ")
